...
call_flow_column_add(ui_t *ui, const char *callid, address_t addr)
{
    ...
    if (setting_enabled(SETTING_ALIAS_PORT)) {
        sng_strlcpy(column->alias, get_alias_value_vs_port(addr.ip, addr.port), sizeof(column->alias));
    } else {
        sng_strlcpy(column->alias, get_alias_value(addr.ip), sizeof(column->alias));
    }
